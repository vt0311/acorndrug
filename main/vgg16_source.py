...
#base_bone_dir = 'C:/work/'
base_bone_dir = 'D:/BoneAge/'
print(os.path.join(base_bone_dir, 'boneage-training-dataset.csv'))
age_df = pd.read_csv(os.path.join(base_bone_dir, 'boneage-training-dataset.csv'))
#age_df['path'] = age_df['id'].map(lambda x: os.path.join(base_bone_dir, '{}.png'.format(x)))
age_df['path'] = age_df['id'].map(lambda x: base_bone_dir + 'boneage-training-dataset/{}.png'.format(x)) 
...
bone_age_model.compile(optimizer = sgd, loss = 'mse',
                           metrics = [mae_months])
